...
// RowsToModel 扫描到结构体。model 必须为一个结构体指针。考虑通用性兼容json tag 必须有json tag才能扫描
func (rs *Rows) RowsToModel(model interface{}) error {
	v := reflect.ValueOf(model)
	if v.Kind() != reflect.Slice || v.Elem().Kind() != reflect.Struct {
	...
	var destScans = make([]interface{}, len(cols))
	for i, c := range cols {
		for j := 0; j < t.NumField(); j++ {
			...
			if tag == c {
				vI := e.Field(j).Addr().Interface()
				destScans[i] = vI
				break
			}
		}
	}
	return rs.Scan(destScans...)
}
...
// ScanToStruct 扫描到结构体。
// model 必须为一个结构体指针。考虑通用性兼容json tag 必须有json tag才能扫描
func (r *Row) ScanToStruct(model interface{}) error {
	rs := r.rows
	cols, err := rs.Columns()
	if err != nil {
		return err
	...
	for i, c := range cols {
		for j := 0; j < t.NumField(); j++ {
			...
			if tag == c {
				destScans[i] = e.Field(j).Addr().Interface()
				break
			}
		}
	}
	return r.Scan(destScans...)
}

// ScanToStruct 扫描到结构体。
// model 必须为一个结构体指针。考虑通用性兼容json tag 必须有json tag才能扫描
func (rs *Rows) ScanToStruct(model interface{}) error {
	cols, err := rs.Columns()
	if err != nil {
		return err
	}
