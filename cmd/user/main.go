...
func main() {
	...
	envPath := flag.String("env", "cmd/user/app-dev.yaml", "specify the configuration")
	...
	app, err := wireinject.InitAppUser(cfg)
	if err != nil {
		panic(err)
	}
	...
	// }()
	go func() {
		mylog.Ctx(ctx).Infof("正在启动rpc服务 codec: %s addr: [:%d] ", app.Cfg.RPCServer.Codec, app.Cfg.RPCServer.Port)
		if err := app.RPCServer.Start(); err != nil {
			mylog.Ctx(ctx).Errorf("rpc服务启动失败！codec: %s %d err: %s", app.Cfg.RPCServer.Codec, app.Cfg.RPCServer.Port, err.Error())
			chanExit <- struct{}{}
		}
	}()
	safeExit(ctx, chanExit, app.RPCServer)
}

// safeExit 优雅的实现程序退出. 退出当前程序不影响下次程序启动，但是会在设定时间内优先处理完当前未完成的链接.
func safeExit(ctx context.Context, chanExit chan struct{}, httpSrv *rpcsrv.Server) {
